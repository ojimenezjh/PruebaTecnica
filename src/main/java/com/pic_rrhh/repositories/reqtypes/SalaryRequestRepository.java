package com.pic_rrhh.repositories.reqtypes;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Repository;

import com.pic_rrhh.dao.reqtypes.DAOSalaryRequest;

@Repository
public interface SalaryRequestRepository extends JpaRepository<DAOSalaryRequest, Integer> {
	
	@Query(value = "UPDATE salary_requests SET accepted= ?2 WHERE id = ?1", nativeQuery = true)
	  public ResponseEntity setSalaryAcceptedValue(int id, boolean acceptedvalue);
	
}